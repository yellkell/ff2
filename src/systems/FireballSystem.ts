...
export class FireballSystem extends createSystem({
  ...
  private updateLocalControl(ball: Entity, hand: Hand, delta: number): void {
    ...
    // Held off on frames where the tutorial owns the trigger (its opening
    // beat, and the frame of a tutorial-console click).
  ...
  private throwBall(ball: Entity, hand: Hand, handSpeed: number): void {
    ...
    v[1] = _dir.y * speed;
    v[2] = _dir.z * speed;
    // THE TAPE: which fist let go, from where on the slab, and how hard.
    telemetry.throw(hand, obj.position.x, obj.position.z, speed);
