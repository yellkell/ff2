/**
 * The placeholder rave — a full techno set synthesised at runtime, and the
 * BEAT CLOCK the entire game runs on.
 *
 * Everything in DANCE RAID is quantized to this clock: telegraphs start on
 * beats, landings hit downbeats, the GOOPLIATH bounces on the kick, the
 * lasers swing per bar. The track itself is seeded — the same seed that
 * choreographs the raid also writes the bassline, so every client hears the
 * same set note-for-note without shipping a byte of audio.
 *
 * When the real tracks are ready, drop them in and start the clock with the
 * same `startSet` contract (bpm + a start time); the game never knows the
 * difference. (See README → "Bringing your own music".)
 *
...
let nextStep = 0; // next 16th-note step index to schedule (counts from the count-in start)
...
function clap(c: AudioContext, t: number): void {
  const b = musicBus(c);
  for (let i = 0; i < 3; i++) {
    const n = c.createBufferSource();
    n.buffer = noise(c);
...
function scheduleStep(c: AudioContext, step: number, t: number): void {
  ...
  if ((act >= 1 || finale) && sixteenth === 0 && (beatInBar === 1 || beatInBar === 3)) clap(c, t);
